...
Checking the cells surrounding the current cell, then checking the cells surrounding _those_ cells, and so on layer by layer — this level-by-level expansion is exactly what BFS does. Since we want the **shortest** path to an exit, BFS is a natural fit: it explores all cells at distance `d` before moving on to distance `d + 1`, so the first exit it reaches is guaranteed to be the nearest one.
...
- **`step` increases by one each time the helper is called for the next layer** — via `depth + 1` . The algorithm itself is iterative (a plain `while` loop over the queue); only the step count advances with each call.
